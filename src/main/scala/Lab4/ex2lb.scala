...
object Ex2{
    def main(args: Array[String]) {
        ...
        val getThreeFour = Flip(0.02)
        val getCards1 = If(getThreeFour, 
            Select(0.2 -> "3 & 4", 0.8 -> "Alta combinatie"),
            Select(0.0 -> "3 & 4", 1.0 -> "Alta combinatie"))
        
        val getTwoOne = Flip(0.02)
        val getCards2 = If(getTwoOne, 
            Select(0.4 -> "2 & 1", 0.6 -> "Alta combinatie"),
            Select(0.0 -> "2 & 1", 1.0 -> "Alta combinatie"))

        val getFiveThree = Flip(0.02)
        val getCards3 = If(getFiveThree, 
            Select(0.3 -> "5 & 3", 0.7 -> "Alta combinatie"),
            Select(0.0 -> "5 & 3", 1.0 -> "Alta combinatie"))
        ...
        val result = VariableElimination.probability(getCards1, "3 & 4")
        println("Probability that we get the 3 & 4 combo: " + result)

        getCards1.observe("3 & 4")
        println(VariableElimination.probability(getThreeFour, true))


        val result1 = VariableElimination.probability(getCards2, "2 & 1")
        println("Probability that we get the 2 & 1 combo: " + result1)

        getCards2.observe("2 & 1")
        println(VariableElimination.probability(getTwoOne, true))

        val result2 = VariableElimination.probability(getCards3, "5 & 3")
        println("Probability that we get the 5 & 3 combo: " + result2)

        getCards3.observe("5 & 3")
        println(VariableElimination.probability(getFiveThree, true))
    }
}