// package Lab4

// import com.cra.figaro.library.atomic.discrete
// import com.cra.figaro.language.Chain
// import com.cra.figaro.library.compound.{RichCPD, OneOf, *}
// import com.cra.figaro.language.{Flip, Constant, Apply}
// import com.cra.figaro.algorithm.factored.VariableElimination

// object Ex3 {
//     def main(args: Array[String]) {
//         // To keep the code simple, I just make the cards an integer
//         val cards = List(5, 4, 3, 2, 1)
//         // The discrete uniform distribution chooses uniformly from a fixed
//         // set of possibilities
//         val player1Card1 = discrete.Uniform(cards:_*)
//         val player1Card2 = Chain(player1Card1, (card: Int) =>
//             // Player 1 can get any card except the first card he got
//             discrete.Uniform(cards.filter(_ != card):_*))
//         val player2Card1 = Chain(player1Card1, (card: Int) =>
//             // Player 2 can get any card except the cards Player1 got
//             discrete.Uniform(cards.filter(_ != card):_*))
//         val player2Card2 = Chain(player1Card2, (card: Int) =>
//             // Player 2 can get any card left
//             discrete.Uniform(cards.filter(_ != card):_*)
//         )
        
//         val player1Bet1 = RichCPD(player1Card1,
//             // Player 1 is more likely to bet with a higher card,
//             // but will sometimes bet with a lower card to bluff
//             OneOf(5, 4, 3) -> Flip(0.9),
//             * -> Flip(0.4) // ×××Change this for part (c)×××
//         )
//         val player1Bet12 = RichCPD(player1Card2,
//             // Player 1 is more likely to bet with a higher card,
//             // but will sometimes bet with a lower card to bluff
//             OneOf(4, 3, 2) -> Flip(0.9),
//             * -> Flip(0.4) // ×××Change this for part (c)×××
//         )

//         val player2Bet = RichCPD(player2Card1, player2Card2, player1Bet1, 
//             (OneOf(5, 4), *) -> Flip(0.9),
//             (*, OneOf(false)) -> Flip(0.5),
//             (*, *) -> Flip(0.1)
//         )
        
//         val player1Bet2 =
//         Apply(player1Card1, player1Card2, player1Bet1, player2Bet,
//             (card1: Int, card2: Int, bet11: Boolean, bet2: Boolean) =>
//             // Player 1’s second bet is only relevant if she passed the
//             // first time and player 2 bet
//             !bet11 && bet2 && (card1 == 5 || card1 == 4) && (card2 == 3 || card2 == 2)
        
//         )
//         // This element represents the gain to player 1 from the game. I have
//         // made it an Element[Double] so I can query its mean.
//         val player1Gain1 = Apply(player1Card1, player2Card1, player1Bet1, player2Bet, player1Bet2,
//             (card1: Int, card2: Int, bet11: Boolean, bet2: Boolean, bet12: Boolean) =>
//                 if (!bet11 && !bet2) 0.0
//                 else if (bet11 && !bet2) 1.0
//                 else if (!bet11 && bet2 && !bet12) -1.0
//                 else if (card1 > card2 ) 2.0
//                 else -2.0
//         )

//          val player1Gain2 = Apply(player1Card2, player2Card2, player1Bet1, player2Bet, player1Bet2,
//             (card1: Int, card2: Int, bet11: Boolean, bet2: Boolean, bet12: Boolean) =>
//                 if (!bet11 && !bet2) 0.0
//                 else if (bet11 && !bet2) 1.0
//                 else if (!bet11 && bet2 && !bet12) -1.0
//                 else if (card1 > card2 ) 2.0
//                 else -2.0
//         )

//         player1Card1.observe(4)
//         player1Card2.observe(5)
//         player1Bet1.observe(true)
//         val alg1 = VariableElimination(player1Gain1)
//         alg1.start()
//         alg1.stop()
//         println("Expected gain for betting:" + alg1.mean(player1Gain1))

//         player1Bet1.observe(false)
//         val alg2 = VariableElimination(player1Gain2)
//         alg2.start()
//         alg2.stop()
//         println("Expected gain for passing:" + alg2.mean(player1Gain2))
//         player1Card1.unobserve()
//         player1Card2.unobserve()
//         player1Bet1.unobserve()

//         player2Card1.observe(3)
//         player2Card2.observe(1)
//         player1Bet1.observe(true)
//         player2Bet.observe(true)
//         val alg3 = VariableElimination(player1Gain1)
//         alg3.start()
//         alg3.stop()
//         println("Expected gain for betting:" + alg3.mean(player1Gain1))

//         player2Bet.observe(false)
//         val alg4 = VariableElimination(player1Gain2)
//         alg4.start()
//         alg4.stop()
//         println("Expected gain for passing:" + alg4.mean(player1Gain2))

//         player2Card1.unobserve()
//         player2Card2.unobserve()
//         player1Bet1.unobserve()
//         player2Bet.unobserve()
//     }
// }