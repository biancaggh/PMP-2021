...
object Tennis
{
	def tennis(probP1ServeWin: Double, probP1Winner: Double, probP1Error: Double, probP2ServeWin: Double, probP2Winner: Double, probP2Error: Double): Element[Boolean] = 
	{
        ...
        def errorPlayer(firstShot: Boolean, secondShot: Boolean, player: Boolean): Element[Boolean] =
        {
            val eroare= if(!firstShot && !secondShot) probP2ServeWin
                           else if (!firstShot && !secondShot && !player) probP1ServeWin
                                else if (player) probP1Winner
										else probP2Winner
            val pError = if (player) probP1Error else probP2Error
			val error = Flip(eroare)
			val perror = Flip(pError)
			If(error,
				Constant(player),
				If(perror, 
					Constant(!player), 
					errorPlayer(false, false, !player)
				)
			)
        }
		...
		def game(p1Serves: Boolean, p1Points: Element[Int], p2Points: Element[Int]): Element[Boolean] =
		{
            ...
            val p1WinsPoint1 = errorPlayer(true,true, p1Serves)
			val newP1Points = Apply(p1WinsPoint,p1WinsPoint1, p1Points,
								(wins: Boolean, wins1:Boolean,  points: Int) =>
									if (wins || wins1) points + 1 else points
			)

			val newP2Points = Apply(p1WinsPoint,p1WinsPoint1, p2Points,
								(wins: Boolean, wins1: Boolean, points: Int) =>
									if (wins || wins1) points else points + 1
			)
